...
watchlist.get("/", async (req, res) => {
  ...
  if (allMovies[0]) {
    ...
    res
      .status(500)
      .json({ payload: "error", success: false, error: "Server error" });
  }
});
...
watchlist.get("/:id", async (req, res) => {
  ...
  if (movie) {
    ...
    res
      .status(404)
      .json({ payload: "not found", success: false, error: "Movie not found" });
  }
});
...
watchlist.put("/:id", async (req, res) => {
  const { id } = req.params;
  const updatedMovie = await updateMovie(req.body, id);
  if (updatedMovie.id) {
